    ...
    <div id="profile-config" hidden
        ...
        data-like-base="{{ url('/project') }}"
        data-auth="{{ auth()->check() ? '1' : '0' }}"
        data-login-url="{{ route('login.required') }}"></div>

    <script>
        const cfg     = document.getElementById('profile-config').dataset;
        const IS_AUTH = cfg.auth === '1';

        // Kirim tamu ke login, lalu kembali ke halaman profil ini
        function goLogin() {
            location.href = cfg.loginUrl + '?next=' + encodeURIComponent(location.pathname + location.search);
        }

        // ===== Like (wajib login, klik lagi = unlike) =====
        ...
        async function toggleLike(btn) {
            ...
            try {
                ...
                if (res.status === 401 || res.status === 419) return goLogin(); // session habis
