        ...
        <div class="mx-auto max-w-7xl px-4 lg:px-8">
            ...
            <div id="allKaryaGrid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                @forelse($karyas as $karya)
                    @php
                        // URL foto siswa (dipakai di modal detail)
                        $av = $karya->user->avatar ?? null;
                        $avUrl = $av
                    ...
                    <div class="karya-card bg-white dark:bg-zinc-900 rounded-3xl overflow-hidden border-3 border-zinc-900 dark:border-zinc-100 shadow-[6px_6px_0px_0px_#18181b] dark:shadow-[6px_6px_0px_0px_#f4f4f5] card-hover transition-all flex flex-col"
                        ...
                        <div class="p-4 flex flex-col flex-grow">
                            ...
                            @if($karya->user)
                                <a href="{{ url('/u/' . $karya->user->id) }}" class="text-[11px] text-sky-600 dark:text-sky-400 font-bold mb-1 w-fit hover:underline">
                                    Oleh {{ $karya->user->name }}
                                </a>
                            @else
                                <p class="text-[11px] text-zinc-500 font-bold mb-1">Oleh Anonim</p>
                            @endif
            ...
            <div class="py-4 space-y-4">
                ...
                <div class="bg-zinc-100 dark:bg-zinc-800 p-4 rounded-2xl border-2 border-zinc-900 dark:border-zinc-100 shadow-[3px_3px_0px_0px_#18181b]">
                    <div class="flex items-center gap-3">
                        {{-- Foto siswa (bulat). Kalau tidak ada foto, tampil huruf pertama nama --}}
                        <div id="modalAvatar" class="relative overflow-hidden w-12 h-12 bg-yellow-300 text-zinc-900 border-2 border-zinc-900 rounded-full flex items-center justify-center font-black text-base shrink-0 shadow-[2px_2px_0px_#000]"></div>
                        <div>
...
<script>
    ...
    // Escape teks sebelum disisipkan lewat innerHTML (mencegah nama/link berisi kode HTML)
    function esc(text) {
    ...
    function openModal(card) {
        ...
        if (actionContainer) {
            ...
            if (d.github) {
                hasButton = true;
                actionContainer.innerHTML += `<a href="${esc(d.github)}" target="_blank" rel="noopener" class="flex-1 py-2.5 bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 font-black text-xs rounded-xl border-2 border-zinc-900 dark:border-zinc-100 shadow-[2px_2px_0px_#000] text-center transition">GitHub</a>`;
            }
            if (d.live) {
                hasButton = true;
                actionContainer.innerHTML += `<a href="${esc(d.live)}" target="_blank" rel="noopener" class="flex-1 py-2.5 bg-yellow-300 text-zinc-900 font-black text-xs rounded-xl border-2 border-zinc-900 shadow-[2px_2px_0px_#000] text-center transition">Live Demo</a>`;
            }
        ...
        if (mediaPreview) {
            if (d.filePath) {
                mediaPreview.innerHTML = `<img src="${esc(d.filePath)}" alt="${esc(d.title)}" class="w-full h-full object-cover" />`;
            } else if (d.live) {
                mediaPreview.innerHTML = `<iframe src="${esc(d.live)}" class="w-full h-full border-0" loading="lazy"></iframe>`;
            } else {
                mediaPreview.innerHTML = `<span class="text-xs font-bold text-zinc-400">Tidak ada preview media</span>`;
            }
        }

        // Avatar siswa: foto kalau ada, kalau tidak (atau gagal dimuat) tampil huruf pertama
        const modalAvatar = document.getElementById("modalAvatar");
        if (modalAvatar) {
            const letter = d.avatarLetter || (d.siswa ? d.siswa.charAt(0).toUpperCase() : "U");
            modalAvatar.innerHTML = "";

            if (d.avatar) {
                const img = document.createElement("img");
                img.src = d.avatar;
                img.alt = d.siswa || "Siswa";
                img.className = "absolute inset-0 w-full h-full object-cover";
                img.onerror = () => { img.remove(); modalAvatar.textContent = letter; };
        ...
        if (siswaContainer) {
            if (d.id) {
                siswaContainer.innerHTML = `<a href="/u/${esc(d.id)}" class="font-black text-xs text-sky-600 dark:text-sky-400 hover:underline">${esc(d.siswa || "-")}</a>`;
            } else {
                siswaContainer.innerHTML = `<p class="font-black text-xs text-zinc-900 dark:text-white">${esc(d.siswa || "-")}</p>`;
            }
        }
