...
@section('content')
    ...
    <div class="flex-1 p-4 sm:p-6 lg:p-8">
        <section class="max-w-4xl mx-auto bg-white neo-border neo-shadow-lg rounded-2xl p-4 sm:p-6 md:p-10">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                        @foreach ($projects as $project)
                            @php
                                ...
                                // Karya tanpa foto/video memakai live_link (link iframe/embed) sebagai preview
                                $iframeUrl = $project->live_link ?? '';
                                $hasIframe = !$isImage && !$isVideo && !empty($iframeUrl);
                            ...
                            <article class="group bg-white neo-border neo-shadow rounded-2xl overflow-hidden flex flex-col justify-between"
                                ...
                                <div>
                                    ...
                                    <div class="relative h-36 flex items-center justify-center bg-[#E0E7FF] border-b-[3px] border-slate-900 overflow-hidden {{ $isVideo ? '' : 'cursor-pointer' }}"
                                        ...
                                        @elseif ($hasIframe)
                                            {{-- Preview iframe: dirender 4x lalu diperkecil 25% agar terlihat seperti miniatur halaman.
                                                 pointer-events-none supaya klik/scroll tidak "tertahan" di dalam iframe. --}}
                                            <iframe src="{{ $iframeUrl }}" title="Preview {{ $project->title }}"
                                                loading="lazy" tabindex="-1" aria-hidden="true"
                                                class="absolute top-0 left-0 w-[400%] h-[400%] origin-top-left scale-25 border-0 bg-white pointer-events-none"></iframe>
                                        @else
