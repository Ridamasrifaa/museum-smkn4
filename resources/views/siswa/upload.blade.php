        ...
        <div class="max-w-2xl w-full bg-white neo-border neo-shadow-lg rounded-2xl p-4 sm:p-6 md:p-8 my-2 sm:my-6">
            ...
            @foreach ($jurusanList as $jurusan)
                ...
                <form id="form_{{ $jurusan }}" method="POST" action="{{ url('/siswa/upload') }}"
                    ...
                    {{-- Nomor Telepon (khusus TJKT & TSM) --}}
                    @if (in_array($key, ['TKJ', 'TSM']))
                        <div>
                            <label class="block text-sm font-extrabold text-slate-800 mb-2">Nomor Telepon / WhatsApp *</label>
                            <input type="tel" inputmode="tel" autocomplete="tel" name="phone_number" required value="{{ $isActiveForm ? old('phone_number') : '' }}"
                                placeholder="Contoh: 081234567890" class="{{ $input }}" />
                            @if ($isActiveForm)
                                @error('phone_number')
                                    <p class="{{ $errorText }}">{{ $message }}</p>
                                @enderror
